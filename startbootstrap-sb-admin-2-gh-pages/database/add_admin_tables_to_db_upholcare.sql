-- Admin Tables for db_upholcare Database
...
-- 3. Add foreign key for booking numbers (check if constraint doesn't exist first)
SET @constraint_exists = (
    SELECT COUNT(*) 
    FROM information_schema.KEY_COLUMN_USAGE 
    WHERE TABLE_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'bookings' 
    AND CONSTRAINT_NAME = 'bookings_booking_number_fk'
);

SET @sql = IF(@constraint_exists = 0, 
    'ALTER TABLE `bookings` ADD CONSTRAINT `bookings_booking_number_fk` FOREIGN KEY (`booking_number_id`) REFERENCES `booking_numbers` (`id`) ON DELETE SET NULL',
...
-- 7. Create reservation_queue table
...
-- 8. Add category_id to services table
...
-- 9. Add foreign key for service categories (check if constraint doesn't exist first)
SET @constraint_exists = (
    SELECT COUNT(*) 
    FROM information_schema.KEY_COLUMN_USAGE 
    WHERE TABLE_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'services' 
    AND CONSTRAINT_NAME = 'services_category_fk'
);

SET @sql = IF(@constraint_exists = 0, 
    'ALTER TABLE `services` ADD CONSTRAINT `services_category_fk` FOREIGN KEY (`category_id`) REFERENCES `service_categories` (`id`) ON DELETE SET NULL',
    'SELECT "Service category foreign key constraint already exists" as message'
);
...
-- 10. Insert sample booking numbers
...
-- 11. Insert default admin settings
...
-- 12. Update existing services with category IDs
...
-- 13. Create admin dashboard stats view
...
-- 14. Create admin booking details view
...
-- 15. Insert existing pending bookings into reservation queue
